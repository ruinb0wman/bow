...
- `Ctrl+Shift+←/→/↑/↓`:在**聚焦的窗格**上分屏(新窗格开一个空白标签并聚焦它,方向 = 新窗格的位置);
  `Alt+Shift+←/→/↑/↓`:让聚焦窗格**朝该方向扩张**(贴到边界就不动,长按可连续调整)
  —— 这两个组合只在**普通网页标签**上接管:地址栏、设置页、终端页、DevTools 前端里的
  `Ctrl+Shift+方向`(按词选择 / xterm 选择扩展)保持原样;代价见下方「标签组与分屏」
- `Ctrl+D` 收藏当前页
- 终端页(`bow://terminal`)里:`Ctrl+W`(删词)与 `Ctrl+L`(清屏)**归 shell** —— 要关终端标签用标签上的 × 或中键;
  想从终端跳回地址栏按 `Ctrl+Shift+L`(不被 shell 占用);
...
- `Alt+Shift+←/→/↑/↓` 让聚焦窗格**朝该方向扩张**(分隔条移动);它已经贴在那层边界上时会**向上一层找**能扩的祖先,
  到最外层边界就不动了。`←/↑` 与 `→/↓` 是同一套语义的两个方向:箭头指着你要推的那条边。
- **在分屏里开终端**:先 `Ctrl+Shift+方向` 分出一个空白窗格,再在地址栏输入 `bow://terminal` ——
  终端**顶替当前聚焦的那个窗格**(不新建标签)。工具栏的终端按钮则是「新标签」入口。
...
- ⚠️ 代价:`Ctrl+Shift+方向` / `Alt+Shift+方向` 是在**所有普通网页标签**上全局接管的,所以网页里的
  `<input>` / `<textarea>` 也拿不到这两个组合(主进程无法同步得知「当前焦点是不是可编辑元素」)。
  地址栏、设置页、终端页、DevTools 前端不受影响。
