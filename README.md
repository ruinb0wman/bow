...
**标签栏里的每一项就是一个标签组**:普通组只有 1 个标签(绝大多数情况),分屏组有 2 个;组里的标签全关掉,这一项就消失。

- 工具栏「分屏」按钮开面板:点一个标签就把它**拼进当前组**的右侧(当前组已满时,原来的非聚焦成员被挤出去、自成一组);
  没有别的标签时点「新建空白标签并在右侧分屏」。候选里可以看到**其它组里的标签** —— 选中就从原组拿走。
- **新建标签(`Ctrl+T` / `+` / MCP `browser_new_tab`)永远是新建一个组,不会拆掉已有的分屏。**
- 分屏组在标签栏里只占**一项**:左右两半各显示自己的标题,点哪半就聚焦哪半(地址栏、前进后退、`Ctrl+L` 都跟着它),
  聚焦的那半有高亮底色。
- `Ctrl+1..9` 切的是**组**(标签栏第几项,`9` = 最后一项),不是按标签切。
- `Ctrl+W` / 标签上的 × / 中键点击 = 关掉**聚焦的那一半**(组降级为单标签,另一个窗格独占整窗);要关两个就按两次。
- **宽度预设**:面板里一排按钮,点一下立即生效(比例档按可用宽度算,所以 `50%` 就是两半等宽;像素档是固定宽度)。
  预设本身在「设置页 → 常规 → 分屏预设」里增删改,落盘到 `settings.json`;面板里一个预设都没有时会给出去设置页添加的提示。
- 「取消分屏」把组拆成**两个单标签组**(两个标签都保留,左右顺序不变)。
- `Esc` 或点面板外的空白处关闭面板;标签组**只在内存**,不跨重启保存(只有预设落盘)。
...
**标签组与分屏**:同一窗口里并排显示两个标签时,它们属于同一个**标签组**,而标签栏的一项就是一个组。
`TabManager` 持有 `groups: TabGroup[]`(纯记账规则在 `@shared/groups`,可单测),`layout()` 是页面视图**可见性的唯一来源**:
只显示活动组的标签,活动组是 2 标签组时按 `computeSplitBounds()`(`@shared/split`)算出的宽度分两窗(两窗之间留 4px 空隙,
渲染层在那条缝上画分隔条);比例档按可用宽度算,像素档夹紧到「两窗都能显示」。新标签总是新建一个组,所以新建标签不会拆掉已有的分屏;
`Ctrl+数字` 按组切。组状态只在内存,预设落盘在 `settings.json`。
