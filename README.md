...
多标签页浏览器:地址栏搜索(输历史/书签实时模糊建议)、浏览历史、书签(文件夹分组)、设置页(内部标签页 `bow://settings`)、终端(内部标签页 `bow://terminal`,连本机 shell)、笔记(内部标签页 `bow://logseq`,直接读写你已有的 Logseq 文件图);内置 **MCP 服务器(stdio)**,AI 编码工具(pi / Claude Code / Cursor 等)可以实时操纵这个浏览器:导航、搜索、点击、输入、滚动、切换标签、截图、读取页面快照。
...
- **块编辑**:点一下块就地编辑(渲染态 → 原始 markdown),`Enter` 新块 / 块中间 `Enter` 劈开 /
  `Shift+Enter` 块内换行 / `Tab` 缩进(成为上一个兄弟的子块)/ `Shift+Tab` 反缩进 / 块首 `Backspace` 合并 /
...
- **不做的事**:`((块引用))` 与 `{{embed}}` 只原样显示(不丢字节);不做 TODO/查询/白板/闪卡、不渲染图里的本地图片
...
- 笔记(「笔记」插件):`<userData>/logseq.json` —— 只存**图目录与最近图**;笔记内容本身就在你的 Logseq 图里,不另存一份
...
- **插件设置**:CORS 放行 / 浏览历史 / 广告追踪拦截 / 终端 / 笔记 等分区直接内联展示(取消旧版的嵌套弹窗)。
  「笔记」分区只**显示**从 `logseq/config.edn` 读到的目录 / 日期格式 / 默认模板与索引统计,并提供「重建索引」——
  改模板请去 Logseq 里改配置(这份插件从不写回 `config.edn`)。
...
书签、历史、CORS 放行、元素全屏、MCP HTTP 服务、默认浏览器、设备检查、终端、笔记都是内置插件;广告/追踪拦截是参考插件。插件是**仓库内编译期模块**(无动态代码执行),
