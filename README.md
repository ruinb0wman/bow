...
工具栏的「分屏」按钮(左右并排图标)把**当前标签**与**另一个标签**并排显示在同一个窗口里(左窗格 = 当前标签):

- 面板里点一个标签 → 在右侧打开它;已分屏时同一位置变成「更换右侧标签」;面板里没有别的标签时,
  直接点「新建空白标签并在右侧分屏」;
- **宽度预设**:面板里一排按钮,点一下立即生效(比例档按可用宽度算,所以 `50%` 就是两半等宽;
  像素档是固定宽度)。预设本身在「设置页 → 常规 → 分屏预设」里增删改,落盘到 `settings.json`;
  面板里一个预设都没有时会给出去设置页添加的提示;
- 点某一半的页面(或在那里滚轮/敲键盘)= 把那一半切成「活动窗格」,地址栏、前进后退、`Ctrl+L` 都跟着它走;
- 点**其它标签**(或 `Ctrl+T` / `Ctrl+Shift+T`)会退出分屏并全屏显示那个标签;
- 关掉分屏里的任意一个标签(标签上的 × / `Ctrl+W`)也会退出分屏,活下来的那半独占整窗;
- `Esc` 或点面板外的空白处关闭面板;分屏状态**不跨重启保存**(只有预设落盘);
- 窗口太窄(放不下两个 120px 的窗格)时会临时退化成单窗格铺满,把窗口拉宽后自动恢复;
- 分屏里的两个窗格可以是任意标签(普通网页、本地文件、`bow://settings`、设备检查的 DevTools 前端都行)。
...
```
src/
  main/          主进程:窗口、TabManager(每标签 WebContentsView + 内部页面标签 + 左右分屏)、
  ...
  shared/        三端共享:类型、URL 解析、内部页面标识、设置页导航模型、分屏预设与两窗格几何、
...
**左右分屏**:同一窗口里并排显示两个标签(工具栏「分屏」按钮,详见「分屏」一节)。`TabManager` 的 `layout()` 是页面视图**可见性的唯一来源**:不分屏时只有活动标签可见,分屏时只有左右两个成员可见、且宽度由 `computeSplitBounds()`(`@shared/split`)按当前预设算出来(两窗之间留 4px 空隙,渲染层在那条缝上画分隔条);比例档按可用宽度算,像素档夹紧到「两窗都能显示」。分屏状态只在内存,预设落盘在 `settings.json`。
