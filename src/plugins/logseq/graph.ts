...
export function parseIndexedFile(input: {
  ...
  const stem = (toPosix(input.rel).split('/').pop() ?? '').replace(MD_RE, '')
  ...
  const day = input.isJournal ? (parseJournalStem(stem, input.format) ?? parseLooseDay(stem)) : null
  const title = titleProp ?? (input.isJournal ? (day ?? decodePageName(stem)) : decodePageName(stem))
  return {
    path: input.path,
...
export function searchPages(index: GraphIndex, query: string, limit = 20): PageHit[] {
  ...
  for (const file of index.files) {
    ...
    hits.push({ name: file.title, rel: file.rel, path: file.path, day: file.day })
  }
  hits.sort((a, b) => {
    const aPrefix = q && pageKey(a.name).startsWith(q) ? 0 : 1
    const bPrefix = q && pageKey(b.name).startsWith(q) ? 0 : 1
