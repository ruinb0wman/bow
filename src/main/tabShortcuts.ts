 ...
 *
 * 多窗口:按键来源的 webContents 先经 `WindowManager.byWebContents()` 反查窗口,
 * 所有动作都作用在**那个窗口**的 tabs/overlay 上;来源认不出(如 DevTools detach 窗口)
 * 时回退到聚焦窗口,仍认不出则直接放行。
 */
...
/**
 * 聚焦地址栏:先把键盘焦点交给该窗口的 chrome WebContents(否则渲染层的 el.focus() 只是改 DOM 状态、
 ...
 * windows/kernel 通过惰性取值,按键事件只会在窗口加载完成后到达,此时实例已就绪。
 */
export function setupTabShortcuts(getWindows: () => WindowManager, getKernel: () => PluginKernel): void {
  app.on('web-contents-created', (_e, contents) => {
    contents.on('before-input-event', (event, input) => {
      // 按键来源 → 所属窗口。认不出(DevTools detach 窗口等)时回退聚焦窗口,再认不出就放行。
       ...
       */
      const closeConfirmOpen = (): boolean => overlay.currentId === CLOSE_CONFIRM_OVERLAY_ID
      ...
      if (splitHk) {
        ...
        if (
          ...
          if (overlay.isFullOpen) return
          // preventDefault 会同时阻止页面 keydown/keyup 与菜单快捷键
          event.preventDefault()
          if (splitHk.kind === 'split') tabs.splitFocused(splitHk.dir)
          else tabs.resizeFocused(splitHk.dir)
